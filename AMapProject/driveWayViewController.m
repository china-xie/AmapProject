...
#define kRouteIndicatorViewHeight   68.f
#define kCollectionCellIdentifier   @"kCollectionCellIdentifier"
@interface driveWayViewController ()<MAMapViewDelegate, AMapNaviDriveManagerDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
@implementation driveWayViewController


- (void)initProperties
{
    if (self.isNowWay) {
        self.startPoint = [AMapNaviPoint locationWithLatitude:self.annotation.latitude longitude:self.annotation.longitude];
        self.endPoint   = [AMapNaviPoint locationWithLatitude:self.poi.location.latitude    longitude:self.poi.location.longitude];
    } else {
    
        self.endPoint= [AMapNaviPoint locationWithLatitude:self.annotation.latitude longitude:self.annotation.longitude];
        self.startPoint = [AMapNaviPoint locationWithLatitude:self.poi.location.latitude    longitude:self.poi.location.longitude];
    }
...
- (void)configSubViews
{
    ...
    self.preferenceView = [[PreferenceView alloc] initWithFrame:CGRectMake(0, 64, CGRectGetWidth(self.view.bounds), 30)];
